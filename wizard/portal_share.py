...
class PortalShare(models.TransientModel):
    ...
    share_type = fields.Selection([
        ('mail', 'Mail'),
        ('whatsapp', 'Whatsapp')
    ], string='Sharing Method', default='mail')
    ...
    def action_send_whatsapp(self):
        '''this function redirects to the whatsapp web with required parameters'''
        if self.note and self.mobile_number:
            if self.res_model == 'sale.order':
                ...
                common_message = ('You have been invited to access the following Purhase.')
            else:
